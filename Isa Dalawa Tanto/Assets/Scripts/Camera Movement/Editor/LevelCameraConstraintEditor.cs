...
[CustomEditor(typeof(LevelCameraConstraint))]
public class LevelCameraConstraintEditor : Editor
{
    public void OnSceneGUI()
    {
        LevelCameraConstraint handler = target as LevelCameraConstraint;
        Transform transform = handler.transform;

        Vector3 leftClampPos = new Vector3(handler.Constraint.x, transform.position.y, transform.position.z);
        Vector3 rightClampPos = new Vector3(handler.Constraint.y, transform.position.y, transform.position.z);
        ...
        if (EditorGUI.EndChangeCheck())
        {
            ...
            handler.Constraint = new Vector2(leftClampHandle.x, rightClampHandle.x);
